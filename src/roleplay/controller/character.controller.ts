...
export class CharacterController {
  ...
  public async messageReply(
    ...
    try {
      ...
      if (!response) {
        this.logger.log("No response generated by character service.");
        return next();
      }
      ...
      this.personService
        .countInteraction(context.chat.id, user.userId)
        .catch((error) =>
          this.logger.error("Failed to count person interaction", error)
        );
    } catch (error) {
      this.logger.error("Failed to process character response", error);
    }
  ...
  async newChatMembers(
    ...
    try {
      ...
      const member = message.new_chat_member as User;
      ...
      if (member.id && !member.is_bot) {
        ...
        if (greet) {
          ...
          throw new Error("No greeting generated by character service.");
        }
      }
    } catch (error) {
      this.logger.error("Failed to process new chat members", error);
    }
  }
